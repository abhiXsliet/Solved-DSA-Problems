...
class Solution:
    ...
    def populateNext(self, root):
        # return self.approach_1(root)
        
        self.prev = None
        return self.approach_2(root)
        
    # TC : O(N)
    # SC : O(1)
    def approach_2(self, root):
        if root is None:
            return
        self.approach_2(root.left)
        if self.prev is not None:
            self.prev.next = root
        self.prev = root
        self.approach_2(root.right)
    
    # TC : O(N)
    # SC : O(N)
    def approach_1(self, root):
        store = []
