...
class Solution{
    private:
    int solve_rec(vector<int> &arr, int n, int idx, int prev) {
        if (idx >= n)
            return 0;
    ...
    int solve_mem(vector<int> &arr, int n, int idx, int prev, vector<vector<int>>& dp) {
        if (idx >= n)
            return 0;
    ...
    int solve_so(vector<int> &arr,int n){
